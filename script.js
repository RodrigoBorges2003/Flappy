/*
 * O Autour foi Menibrasil
 * https://github.com/Menibrasil/MenibrasilNetCommonJS
 */


(function (window){

  var Menibrasil = window.Menibrasil || {};
  window.Menibrasil = window.Menibrasil || Menibrasil;
    
	Menibrasil.MathUtil = {};
	
	//usado para radianos em graus e graus
	Menibrasil.MathUtil.PI_180 = Math.PI/180;
	Menibrasil.MathUtil.ONE80_PI = 180/Math.PI;
	
	//pré-cálculos para valores de 90, 270 e 360 ​​em radianos
	Menibrasil.MathUtil.PI2 = Math.PI*2;
	Menibrasil.MathUtil.HALF_PI = Math.PI/2;


	//retornar número entre 1 e 0
	Menibrasil.MathUtil.normalize = function(value, minimum, maximum){
		return (value - minimum) / (maximum - minimum);
	};

	//mapear número normalizado para valores
	Menibrasil.MathUtil.interpolate = function(normValue, minimum, maximum){
		return minimum + (maximum - minimum) * normValue;
	};

	//mapear um valor de um conjunto para outro
	Menibrasil.MathUtil.map = function(value, min1, max1, min2, max2){
		return Menibrasil.MathUtil.interpolate( Menibrasil.MathUtil.normalize(value, min1, max1), min2, max2);
	};

	Menibrasil.MathUtil.getRandomNumberInRange = function(min, max){
		return min + Math.random() * (max - min);
	};
	
	Menibrasil.MathUtil.getRandomIntegerInRange = function(min, max){
		return Math.round(Menibrasil.MathUtil.getRandomNumberInRange(min, max));
	};
...
(function (window){
    ...
    Menibrasil.Geom.Point = function (x,y){
        this.x = isNaN(x) ? 0 : x;
        this.y = isNaN(y) ? 0 : y;
    };

    Menibrasil.Geom.Point.prototype.clone = function(){
        return new Menibrasil.Geom.Point(this.x,this.y);
    };

    Menibrasil.Geom.Point.prototype.update = function(x, y){
        this.x = isNaN(x) ? this.x : x;
        this.y = isNaN(y) ? this.y : y;
    };

    Menibrasil.Geom.Point.prototype.equals = function(point){
        return this.x==point.x && this.y==point.y;
    };

    Menibrasil.Geom.Point.prototype.toString = function(){
	...
	Menibrasil.Geom.Rectangle = function (x, y, width, height){
		this.update(x, y, width, height);
	};
	
	Menibrasil.Geom.Rectangle.prototype.update = function(x, y, width, height){
		this.x = isNaN(x) ? 0 : x;
		this.y = isNaN(y) ? 0 : y;
	...
	Menibrasil.Geom.Rectangle.prototype.getRight = function(){
		return this.x + this.width;
	};
	
	Menibrasil.Geom.Rectangle.prototype.getBottom = function(){
		return this.y + this.height;
	};

    Menibrasil.Geom.Rectangle.prototype.getCenterX = function(){
        return this.x + this.width/2;
    };

    Menibrasil.Geom.Rectangle.prototype.getCenterY = function(){
        return this.y + this.height/2;
    };

    Menibrasil.Geom.Rectangle.prototype.containsPoint = function(x, y){
        return x >= this.x && y >= this.y && x <= this.getRight() && y <= this.getBottom();
    };

	
	Menibrasil.Geom.Rectangle.prototype.clone = function(){
		return new Menibrasil.Geom.Rectangle(this.x, this.y, this.width, this.height);
	};
	
	Menibrasil.Geom.Rectangle.prototype.toString = function(){
...
(function (window){
    ...
    Menibrasil.CanvasTextUtil = {};

    //retornar o maior tamanho de fonte que melhor se ajusta ao retângulo
    Menibrasil.CanvasTextUtil.getFontSizeForRect = function(string, fontProps, rect, canvas, fillStyle){
        if(!canvas){
    ...
    //=========================================================================================
    //==============::PROPRIEDADES DO TEXTO DA TELA::====================================
    //========================================================

    Menibrasil.CanvasTextProperties = function(fontWeight, fontStyle, fontSize, fontFace){
    ...
    Menibrasil.CanvasTextProperties.NORMAL = "normal";
    Menibrasil.CanvasTextProperties.BOLD = "bold";
    Menibrasil.CanvasTextProperties.BOLDER = "bolder";
    Menibrasil.CanvasTextProperties.LIGHTER = "lighter";

    Menibrasil.CanvasTextProperties.ITALIC = "italic";
    Menibrasil.CanvasTextProperties.OBLIQUE = "oblique";


    Menibrasil.CanvasTextProperties.prototype.setFontWeight = function(fontWeight){
        switch (fontWeight){
            case Menibrasil.CanvasTextProperties.NORMAL:
            case Menibrasil.CanvasTextProperties.BOLD:
            case Menibrasil.CanvasTextProperties.BOLDER:
            case Menibrasil.CanvasTextProperties.LIGHTER:
                this.fontWeight = fontWeight;
                break;
            default:
                this.fontWeight = Menibrasil.CanvasTextProperties.NORMAL;
        }
    };

    Menibrasil.CanvasTextProperties.prototype.setFontStyle = function(fontStyle){
        switch (fontStyle){
            case Menibrasil.CanvasTextProperties.NORMAL:
            case Menibrasil.CanvasTextProperties.ITALIC:
            case Menibrasil.CanvasTextProperties.OBLIQUE:
                this.fontStyle = fontStyle;
                break;
            default:
                this.fontStyle = Menibrasil.CanvasTextProperties.NORMAL;
        }
    };

    Menibrasil.CanvasTextProperties.prototype.setFontSize = function(fontSize){
        if(fontSize && fontSize.indexOf && fontSize.indexOf("px")>-1){
            var size = fontSize.split("px")[0];
            fontProperites.fontSize = isNaN(size) ? 24 : size;//24 é apenas um número arbitrário
            return;
        }
        this.fontSize = isNaN(fontSize) ? 24 : fontSize;//24 é apenas um número arbitrário
    };

    Menibrasil.CanvasTextProperties.prototype.clone = function(){
        return new Menibrasil.CanvasTextProperties(this.fontWeight, this.fontStyle, this.fontSize, this.fontFace);
    };

    Menibrasil.CanvasTextProperties.prototype.getFontString = function(){
...
//========================
//propriedades gerais para configuração de demonstração
//========================
...
// Lembrar de não executar scripts de redimensionamento repetidamente se uma janela do navegador estiver sendo redimensionada arrastando
...
function commitResize(){
    ...
    htmlBounds = new Menibrasil.Geom.Rectangle(0,0, getWidth(canvasContainer) , getHeight(canvasContainer));
    ...
    bounds = new Menibrasil.Geom.Rectangle(0,0, canvas.width, canvas.height);
    context.clearRect(0,0,canvas.width, canvas.height);
    ...
    var textInputSpanY = (canvas.height - canvas.height*.85)/2 + 15;//15 é uma estimativa para metade de textInputHeight
...
function stageTooSmallHandler(){
    var warning = "Desculpe, é necessária uma tela maior :(";
    context.font = "normal 24px sem serifa";
...
//========================
//Propriedades específicas de demonstração
//========================
    ...
    var scrollSpeed = 3;
    var score;
    var fontProperties = new Menibrasil.CanvasTextProperties(Menibrasil.CanvasTextProperties.BOLD, null, 100);
    ...
    function renderGameOver(){
        ...
        var instruction = "Clique para reiniciar.";
        context.font = "negrito normal 24px sem serifa";
    ...
    function createLogo(logoText, logoCanvas, logoCanvassBG){
        ...
        var textRect = new Menibrasil.Geom.Rectangle(0, 0, logoCanvas.width * .8, logoCanvas.height);
        var logoFontProps = fontProperties.clone();
        logoFontProps.fontSize = Menibrasil.CanvasTextUtil.getFontSizeForRect(logoText, fontProperties, textRect);
    ...
    var currentTube;
    var isHit = false;
    var ffScoreBugFix = 0;// por algum motivo, a pontuação dispararia várias vezes no Firefox
    ...
    var collisionPoint = new Menibrasil.Geom.Point();
    var birdPoints = [];
    ...
    var characters;
    var birdFontProperties = new Menibrasil.CanvasTextProperties(Menibrasil.CanvasTextProperties.BOLD, null, 50);
    ...
    function createTubes(){
        ...
        for(var i = 0; i < totalTubes; i++){
            ...
            tubes[i].topRect = new Menibrasil.Geom.Rectangle(canvas.width+(i * tubesGapWidth));
            tubes[i].bottomRect = new Menibrasil.Geom.Rectangle(canvas.width+(i * tubesGapWidth));
            renderTube(tubes[i]);
        }
        currentTube = tubes[0];
    }
    ...
    function renderTube(tube){
        ...
        tube.bottomRect.height = groundGraphicRect.y - tube.bottomRect.y - 1;//menos um para acidente 
...
function createCityGraphic(){
    ...
    for(iterator=0; iterator<canvas.width; iterator+=minCloudRadius){
        cgContext.beginPath();
        cgContext.arc( iterator , maxCloudRadius, Menibrasil.MathUtil.getRandomNumberInRange(minCloudRadius, maxCloudRadius), 0, Menibrasil.MathUtil.PI2);
        cgContext.closePath();
        cgContext.fill();
    }
    ...
    for(iterator=0; iterator<canvas.width; iterator+=(houseWidth+8)){
        ...
        houseHeight = Menibrasil.MathUtil.getRandomNumberInRange(cityGraphicHeight *.5 , cityGraphicHeight - maxCloudRadius *.8);
    ...
    for(iterator=0; iterator<canvas.width; iterator+=minTreeRadius){
        cgContext.beginPath();
        radius = Menibrasil.MathUtil.getRandomNumberInRange(minCloudRadius, maxCloudRadius)
        cgContext.arc( iterator , cityGraphicHeight, radius, 0, Menibrasil.MathUtil.PI2);
        cgContext.closePath();
        cgContext.fill();
    ...
    var sand = "#dcd795";
    var groundGraphicRect = new Menibrasil.Geom.Rectangle();
